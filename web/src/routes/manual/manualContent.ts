          ...
          blocks: [
            {
              ...
              items: [
                ...
                'Submitting immediately emails every active manager at your own building. You get no confirmation email yourself. One exception: if the whole request is already in the past — yesterday’s sick day — NO email goes out at all; the request just sits in the queue until a manager opens it. See "Known gaps".',
        ...
        // ------------------------------------------------------------------
        {
          ...
          blocks: [
            ...
            },
            {
              ...
              text:
                'Clicking an engineer’s chip in the roll opens a small form pre-set for the morning call-out: Sick, source Phone call, reason "called out", that engineer’s own day length in hours, that one day only. Switch the type away from Sick and the "called out" reason clears itself. Anything logged this way saves as APPROVED immediately — it never reaches the queue. Entering a partial-day time halves the hours automatically, unless you typed your own figure first. Switching the type to Vacation runs the same 2-engineer cap check as Add PTO: if the cap is already met you must type an override reason, and the entry saves with the OVERRIDE badge.',
            },
            {
              kind: 'note',
              tone: 'warn',
          ...
          blocks: [
            ...
            },
            {
              ...
              items: [
                ...
                'Quiet hours: anything that would send between 9pm and 6am Eastern waits in a queue and is delivered shortly after 6:00am instead — nothing dings a phone overnight. This covers every send: manager notices, the engineer’s personal copy, calendar invites and the shared-calendar sync. Approving PTO at 11pm is fine; the emails simply arrive at 6am. The window is adjustable by an admin without a redeploy.',
          ...
          blocks: [
            ...
            },
            {
              ...
              items: [
                ...
                'The reviewer’s name appears on approved and denied rows in the year logs — the manager balances drill-down and the engineer’s own My time off log. The denial reason is far less visible: it survives only as a hover tooltip on that "by …" text in those same rows — no mouse hover, no reason — and appears nowhere else. The decision email is still the only copy that spells the reason out in plain text.',
                ...
                'A manager with no home site set on their profile receives NO PTO emails for any site, silently. An ENGINEER with no home site is treated as UPark. Adding someone from the Binney admin stamps their home site automatically; the UPark add form still leaves it blank — fill it in.',
                ...
                'In the Edit form, hours re-compute when you change the dates — but typing your own figure switches them to manual for the rest of that edit, and partial-day rows never auto-sync. Glance at the hours before saving a date change on either.',
                'The attendance roll runs on APPROVED entries only. An engineer with a pending request for tomorrow shows as a green "in" chip and counts as present in the headcount, while the heatmap beside it is already counting that same pending request. The two widgets will disagree until you work the queue.',
                'Balance figures fall back to the newest year on record. After the calendar rolls over and before new allotments are seeded, the pending-card balance line and the Add / Quick / Edit balance cards quietly show LAST year’s numbers — the cards name the old year in small print, the pending line doesn’t say at all. "No balance set" only appears for someone with no row in ANY year.',
              ],
            },
          ],
        },
